...
# Routes لخدمة الملفات الثابتة (HTML, CSS, JS)
@app.route('/')
def serve_index():
    try:
        ...
        try:
            ...
            return f"""
            <html>
                <head><title>PayMe System</title></head>
                <body>
                    <h1>🚀 نظام PayMe يعمل!</h1>
                    <p>الخادم يعمل بشكل صحيح، لكن ملف الواجهة غير موجود.</p>
                    <p>📁 المسار الحالي: {BASE_DIR}</p>
                    <p>📁 مسار الملفات الثابتة: {STATIC_DIR}</p>
                    <hr>
                    <h3>✅ واجهة برمجة التطبيقات (API) تعمل:</h3>
                    <a href="/api">تفقد واجهة API</a><br>
                    <a href="/api/exchange-rate">أسعار الصرف</a><br>
                    <a href="/test">صفحة الاختبار</a>
                    <hr>
                    <h3>📁 الملفات الموجودة:</h3>
                    <ul>
                        {"".join([f'<li>{f}</li>' for f in os.listdir(BASE_DIR) if f.endswith('.html') or f.endswith('.css') or f.endswith('.py')])}
                    </ul>
                </body>
            </html>
            """, 404
...
# Route أساسي للتأكد من أن الخادم يعمل (API فقط)
@app.route('/api')
def api_home():
    return jsonify({
...
if __name__ == '__main__':
    ...
    print(f"📁 مسار الملفات الثابتة: {STATIC_DIR}")
