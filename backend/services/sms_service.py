...
class SimulatedSMSService:
    ...
    def send_otp(self, phone_number, card_last4):
        ...
        try:
            ...
            if encrypted_phone in self.failed_attempts:
                if self.failed_attempts[encrypted_phone] >= 5:
                    return False, 'تعداد درخواست‌ها بیش از حد مجاز است'
...
🌐 payme.ir
            ...
            cursor.execute(
                ...
                (phone_number, message, otp)
            )
            conn.commit()
            conn.close()
            
            # ذخیره در حافظه برای تأیید
